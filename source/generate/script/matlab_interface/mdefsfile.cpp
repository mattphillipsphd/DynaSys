...
MDefsFile::MDefsFile(const std::string& name)
    : MFileBase(name)
{
}
...
void MDefsFile::Make(std::ofstream& out) const
{
    ...
    out <<
//            "function [measure, input_names, inputs, bounds, is_par, output_names, tau] = "
            "function xInfo = " + name_m + "()\n"
    ...
    out << "xInfo.input_names = input_names;\n";
    out << "\n";
    ...
    out << "xInfo.bounds = bounds;\n";
    out << "\n";

    //A boolean vector describing which of the inputs are (potentially optimizable) parameters
    out <<
            "xInfo.is_par = false(" + num_all_inputs_str + ", 1);\n"
            "xInfo.is_par(" + std::to_string(NUM_AUTO_ARGS+1) + ":" + std::to_string(NUM_AUTO_ARGS+num_inputs)
                + ") = true;\n";
    out << "\n";
    ...
    out << "xInfo.tau = " + std::to_string(_modelMgr->ModelStep()) + ";\n";
    out << "\n";
